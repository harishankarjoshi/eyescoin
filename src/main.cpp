...
#if defined(NDEBUG)
# error "Eyescoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x546ae3af19770d34a04675d720af6cf41250c7b019b4b73d110285ca230be4fe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Eyescoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Eyescoin Signed Message:\n";
...
{
    // Eyescoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 8400 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 8400); // Eyescoin: 8.40k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Eyescoin: 1 days
static const int64 nTargetSpacing = 1 * 30; // Eyescoin: 30 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Eyescoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Eyescoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc3696f4948cd2adb50b34e81a91fdaad1caf5899694ba32e6331cab4a2881138");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is eyescoin core for ezuhelmy.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0460f03f2ca7ec80797f9b2250a363459f2a0f23b5614977c6307630e19bd4aa946181e10e952ed9a05f7a0665dd9c64c31d6e9de808e775c1f32931f2a92e038b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528970379;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084925825;

        if (fTestNet)
        {
            block.nTime    = 1528970155;
            block.nNonce   = 385539841;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd024bce06d3147e65ab82e435cf3e1014f6ea4b425d0f828caeae6d72b95abb3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc1, 0xb4, 0xd3 }; // Eyescoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EyescoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EyescoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EyescoinMiner(CWallet *pwallet)
{
    printf("EyescoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eyescoin-miner");
    ...
    try { loop {
        ...
        printf("Running EyescoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EyescoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EyescoinMiner, pwallet));
}
